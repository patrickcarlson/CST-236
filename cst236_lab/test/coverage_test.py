...
class coveragejobstories(TestCase):
    ...
    @story(['Any time someone asks "What is the smallest amount of coins that can be returned for : <cash>?" the application should read back " x quarters, x dimes, x nickels, and x pennies."'])
    def test_coin_count(self):
        qaobject = Interface()
        result = qaobject.ask("What is the smallest amount of coins that can be returned for: 1.75?")
        self.assertEqual(result, "7 quarters, 0 dimes, 0 nickels, and 0 pennies.")
        result = qaobject.ask("What is the smallest amount of coins that can be returned for: 1.23?")
        self.assertEqual(result, "4 quarters, 2 dimes, 0 nickels, and 3 pennies.")

    @story(['Any time someone asks "What are the angle measurements of a right triangle with side lengths: 3 4 5?" the applications should return " <angle> degrees, <angle> degrees, and <angle> degrees"'])
    def test_right_triangle_angles(self):
        qaobject = Interface()
        result = qaobject.ask("What are the angle measurements of a right triangle with side lengths: 3 4 5?")
        self.assertEqual(result, "90.0 degrees, 53.13 degrees, and 36.87 degrees")
        result = qaobject.ask("What are the angle measurements of a right triangle with side lengths: 10 10 14.1?")
    ...
    def test_get_velocity_object_dropped(self):
        qaobject = Interface()
        result = qaobject.ask("What is the velocity of a rock dropped from 10 meters just before it hits the ground?")
        self.assertEqual(result, 14)

    @story(['Any time someone asks "What is the boiling temperature, in degrees fahrenheit, at <elevation> feet?" the application should return the temperature, in degrees at which water will boil'])
    def test_get_boiing_elevation(self):
        qaobject = Interface()
        result = qaobject.ask("What is the boiling temperaturem in degrees fahrenheit, at 10000 feet?")
